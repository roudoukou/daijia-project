...
进入redis容器内部
```shell
docker exec -it redis-daijia redis-cli --raw
flushdb
keys *


GEOADD driver:geo:location 116.403963 39.915119 tiananmen 116.417876 39.915411 wangfujing 116.404354 39.904748 qianmen
#搜索范围内的用户
GEORADIUS driver:geo:location 116.4000 39.9000 1 km WITHDIST
```
...
```
key：指定存储地理空间数据的键。
longitude：位置的经度，范围是 -180 到 180。
latitude：位置的纬度，范围是 -85.05112878 到 85.05112878。
member：与经纬度关联的成员名称。
...
```
key：地理空间集合的键名。
longitude 和 latitude：指定查询圆心的经纬度。
radius 和 unit：指定半径大小和单位：
单位可以是：
* m（米）
* km（千米）
* mi（英里）
